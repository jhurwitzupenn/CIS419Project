...
Current research efforts in the area of shortest path algorithms have focused on specific types of graphs, and suggest cases for when a specific algorithm should be used based on characteristics of the input graph. Researchers F. Benjamin Zhan and Charles E. Noon precomputed key characteristics of real road networks in their work to find the fastest shortest path algorithm \cite{Zhan98} te. However, this means today, users must either know about specific features of the graph to decide which algorithm to use, or they must choose an algorithm at random. 
