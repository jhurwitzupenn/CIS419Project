...
\begin{abstract} 
We implemented a classifier to determine the most efficient shortest path algorithm for a given road network in the context of of optimizing the response times of emergency responder units. We explore the viability of this option by training three key classifiers, including two variations on a Support Vector Machine (SVM) classifiers and a standard decision tree. In particular, we trained these classifiers on variations of Djikstra's shortest path algorithm and the A$\star$ algorithm. We find that our ultimate choice of XXXXXXX showed promise in most efficiently identifying an appropriate shortest path algorithm to use, such that an appropriate path is output. These classifiers draw upon principles from Machine Learning and are relevant for our target application of optimizing emergency responder times. 
...
The training and test datasets are sourced from the Dryad Digital Repository under the title \textit{Barrington-Leigh-Millard-Ball-PNAS2015-Century-of-sprawl-graphs-in-GML-format}. There were hundreds of graphs, each with thousands of nodes, all in GML format. * Add size of training and test sets.
